...
(defn compute-new-topology->executor->node+port [nimbus existing-assignments topologies scratch-topology-id]
  (let [conf (:conf nimbus)
        ...
        supervisor-details (basic-supervisor-details-map storm-cluster-state)
		reassign-details (StringBuilder.)]
    ...
    (doseq [[topology-id executor->node+port] new-topology->executor->node+port
            :let [old-executor->node+port (-> topology-id
                  ...
                  reassignment (filter (fn [[executor node+port]]
                                       ...
                                       executor->node+port)
				  old-node->host (-> topology-id existing-assignments :node->host)
                  new-node->host (->> executor->node+port vals (map first) set
										(mapcat (fn [node] 
													(if-let [host (.getHostName (:inimbus nimbus) supervisor-details node)][[node host]])))
										(into {}))]]
      (when-not (empty? reassignment)
        (let [new-slots-cnt (count (set (vals executor->node+port)))
              reassign-executors (keys reassignment)
			  topology (.getById topologies  topology-id)]
			  (doseq [executor reassign-executors
					:let [executor-detail (ExecutorDetails. (first executor) (last executor)) 
						  old-node+port (get old-executor->node+port executor) 
						  new-node+port (get executor->node+port executor)]]  
				(.append reassign-details 
					(str "Executor " (get (.getExecutorToComponent topology) executor-detail) " " executor-detail
						" in topology " (.getName topology) 
						" is reassigned from " (get old-node->host (first old-node+port)) ":" (second old-node+port) 
						" to " (get new-node->host (first new-node+port)) ":" (second new-node+port) ". ")))
          (log-message "Reassigning " topology-id " to " new-slots-cnt " slots")
          (log-message "Reassign executors: " (vec reassign-executors)))))
	(if (> (.length reassign-details) 0)
        (log-warn (.toString reassign-details) "$$$ReassignAlert" ))
