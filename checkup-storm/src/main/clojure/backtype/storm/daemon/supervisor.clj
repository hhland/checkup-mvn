  ...
  (let [local-state (worker-state conf id)]
    (try
      ...
      (catch RuntimeException e
        (let [streamCorruptedException (instance? java.io.StreamCorruptedException (.getCause e))]
          (if streamCorruptedException
            (log-message "Worker heartbeat data corrupted, let supervisor try to clean up this worker")
            (throw e)))
        ))
    ))
...
(defn mk-synchronize-supervisor [supervisor sync-processes event-manager processes-event-manager]
  (fn this []
    (let [conf (:conf supervisor)
      ...
      ;; only shutdown workers that in ":disallowed" state instead of shuting down all workers. 
      ;; Some wokers may be from other running topologies(not from the killing one), should not have been killed.
...
(defmethod launch-worker
    ...
    (let [conf (:conf supervisor)
          ...
          childopts (.replaceAll (str (conf WORKER-CHILDOPTS) " " (storm-conf TOPOLOGY-WORKER-CHILDOPTS))
                                 "%ID%"
                                 (str port))
          jmx-port-offset (System/getProperty "storm.worker.jmxremote.port.offset")
          jmx-port (if (nil? jmx-port-offset) (+ port 1000) (+ port (Integer. jmx-port-offset)))
          logfilename (str "worker-" port ".log")
          command (str "java -server " childopts
                       " -Djava.library.path=" (conf JAVA-LIBRARY-PATH)
                       " -Dlogfile.name=" logfilename
                       " -Dstorm.home=" (System/getProperty "storm.home")
                       " -Dstorm.log.dir=" (System/getProperty "storm.log.dir")
                       " -Dlogback.configurationFile=" (System/getProperty "logback.configurationFile")
                       " -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.authenticate=false"
                       " -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.port=" jmx-port
