...
(defn ui-template [body]
  (html
    ...
    [:body
     [:h1 (link-to "/" "Storm UI") (link-to "/topman.html" " (Submit Topology) ")]
     (seq body)
     ]))
...
(defroutes main-routes
  ...
  (ring.middleware.multipart-params/wrap-multipart-params
    (POST "/upload" [:as {params :params} :as {headers :headers}]
      (let [file-params (params :file)
            src (if-not (nil? name) (file-params :filename) name)
            dest (. ( new java.io.File (file-params :filename)) getName) ]
        (log-message (str "upload from " src " to " dest))
        (if on-windows? (sh "cmd" "/c" "copy" "/y" src dest)
          (sh "cp" "-f" src dest)))
      (resp/redirect (get headers "referer"))))
  (POST "/submit/:jar/:class/:args" [:as {params :params} :as {headers :headers} jar class args]
    (let [cmd (apply str[ (if on-windows? "bin\\storm.cmd" "bin/storm")
  ...
  (POST "/dir/:args" [:as {cookies :cookies} args]
    (let [jarFiles  "*.jar"]
      (let [jars-str (if on-windows? (:out (sh  "cmd" "/c" "dir" "/b" jarFiles))
        (:out (sh "ls" jarFiles args)))
            jars (clojure.string/split jars-str (if on-windows? #"\r\n" #"\n"))]
        jars-str))  )

  (POST "/delete/:args" [:as {cookies :cookies} args :as {headers :headers}]

    (let [cmd (apply str [(if on-windows? "cmd /c del " "rm ") args (if on-windows? " /s /f /q" "") ])]
      (log-message cmd)
      (exec-command! cmd)
      (resp/redirect (get headers "referer"))))
